...
describe("printer", () => {
  ...
  describe("const/var/override/alias declarations", () => {
    ...
    it("formats var<storage, read_write> with atomic type", async () => {
      const result = await fmt("@group(0) @binding(4) var<storage,read_write> globalMaxHits:atomic<u32>;");
      expect(result).toBe("@group(0) @binding(4) var<storage, read_write> globalMaxHits: atomic<u32>;\n");
    });
  ...
  describe("struct declarations", () => {
    ...
    it("formats struct with members and trailing commas", async () => {
      const result = await fmt("struct Vertex{@location(0) position:vec3f,@location(1) color:vec4f}");
      expect(result).toBe("struct Vertex {\n  @location(0) position: vec3f,\n  @location(1) color: vec4f,\n}\n");
    });

    it("formats atomic struct members", async () => {
      const result = await fmt("struct Pixel{r:atomic<u32>,g:atomic<u32>,b:atomic<u32>,hits:atomic<u32>,}");
      expect(result).toBe("struct Pixel {\n  r: atomic<u32>,\n  g: atomic<u32>,\n  b: atomic<u32>,\n  hits: atomic<u32>,\n}\n");
    });

    it("formats fixed-size array struct members", async () => {
      const result = await fmt("struct S{indices:array<u32,20>,weights:array<f32,20>,}");
  ...
  describe("statements", () => {
    ...
    it("formats continue inside if inside for loop", async () => {
      const result = await fmt("fn f(){for(var i=0u;i<10u;i++){if i<5u{continue;}}}");
  ...
  describe("blank lines between declarations", () => {
    ...
    it("preserves intentional blank line between same-kind declarations", async () => {
...
`;
    ...
    it("does not add blank line between same-kind declarations", async () => {
      const result = await fmt("const A = 1;\nconst B = 2;\n");
      expect(result).toBe("const A = 1;\nconst B = 2;\n");
    });

    it("collapses multiple blank lines between same-kind declarations to one", async () => {
      const input = `const A = 1;
...
`);
    ...
    it("is idempotent for blank line between same-kind declarations", async () => {
      await assertIdempotentAtAllWidths(`const A = 1;
...
`);
  ...
  describe("binary expression wrapping", () => {
    ...
    it("wraps long chain of same-precedence binary ops", async () => {
      ...
      const result = await fmt("const x = alpha + bravo + charlie + delta + echo + foxtrot + golf + hotel + india;", 80);
    ...
    it("is idempotent for wrapped binary expressions", async () => {
      await assertIdempotentAtAllWidths("const x = alpha + bravo + charlie + delta + echo + foxtrot + golf + hotel + india;");
    });
...
`;
    ...
    it("is idempotent for blank line preservation", async () => {
      await assertIdempotentAtAllWidths(`fn f() {
  let x = 1;
...
`);
  ...
  describe("large switch", () => {
    it("formats 30-case switch with u-suffixed selectors", async () => {
      const cases = Array.from({ length: 30 }, (_, i) => `case ${i}u:{return fn_${i}(v,m,r);}`).join("");
  ...
  describe("template argument wrapping", () => {
    it("keeps short template args inline", async () => {
      const result = await fmt("var x: vec4<f32>;");
      expect(result).toBe("var x: vec4<f32>;\n");
    });

    it("keeps short call template args inline", async () => {
      const result = await fmt("const x = array<f32, 4>(1.0, 2.0, 3.0, 4.0);");
      expect(result).toBe("const x = array<f32, 4>(1.0, 2.0, 3.0, 4.0);\n");
    });

    it("wraps long type template args", async () => {
      const result = await fmt("var x: some_very_long_type_name<very_long_arg_one, very_long_arg_two, very_long_arg_three>;", 80);
      expect(result).toBe(
        `var x: some_very_long_type_name<
  very_long_arg_one,
  very_long_arg_two,
  very_long_arg_three,
>;
`,
    ...
    it("wraps long call template args", async () => {
      const result = await fmt("const x = some_very_long_func_name<very_long_arg_one, very_long_arg_two, very_long_arg_three>(a);", 80);
      expect(result).toBe(
        `const x =
  some_very_long_func_name<
    very_long_arg_one,
    very_long_arg_two,
    very_long_arg_three,
  >(a);
`,
    ...
    it("is idempotent for wrapped type templates", async () => {
      await assertIdempotentAtAllWidths("var x: some_very_long_type_name<very_long_arg_one, very_long_arg_two, very_long_arg_three>;");
    });

    it("is idempotent for wrapped call templates", async () => {
      await assertIdempotentAtAllWidths(
        "const x = some_very_long_func_name<very_long_arg_one, very_long_arg_two, very_long_arg_three>(a);",
      );
    });
  });

  describe("member access chain breaking", () => {
    it("keeps short chain flat", async () => {
      const result = await fmt("fn f() { let x = a.b.c; }");
      expect(result).toBe("fn f() {\n  let x = a.b.c;\n}\n");
    });

    it("keeps short mixed chain flat", async () => {
      const result = await fmt("fn f() { let x = a.b[0].c; }");
      expect(result).toBe("fn f() {\n  let x = a.b[0].c;\n}\n");
    });

    it("wraps long member chain at dot boundaries", async () => {
      const result = await fmt(
        "fn f() { let x = some_object.first_field_name.second_field_name.third_field_name.fourth_field_name.fifth_field_name; }",
        80,
      );
      expect(result).toBe(
        `fn f() {
...
`,
    ...
    it("wraps long mixed member/index chain", async () => {
      const result = await fmt(
        "fn f() { let x = some_object.first_field_name[some_long_index].second_field_name.third_field_name.fourth_field_name; }",
        80,
      );
      expect(result).toBe(
        `fn f() {
  let x =
    some_object
      .first_field_name[some_long_index]
      .second_field_name
      .third_field_name
      .fourth_field_name;
}
`,
    ...
    it("is idempotent for wrapped member chains", async () => {
      await assertIdempotentAtAllWidths(
        "fn f() { let x = some_object.first_field_name.second_field_name.third_field_name.fourth_field_name.fifth_field_name; }",
      );
    });
  ...
  describe("comment placement edge cases", () => {
    it("preserves comment before function with attributes", async () => {
      const input = `// this is the main function
@vertex
fn main() -> @builtin(position) vec4f {
  return vec4f(0.0);
}
`;
    ...
    it("preserves trailing comment on struct member", async () => {
...
`;
    ...
    it("preserves comment between function parameters", async () => {
      const input = `fn f(
  // first param
  a: f32,
  // second param
...
`;
    ...
    it("preserves comment before else clause", async () => {
      const input = `fn f() {
  if x {
    return;
  }
...
`;
      const result = await fmt(input);
      // Prettier moves the comment to trailing on else keyword
      expect(result).toBe(`fn f() {
  if x {
    return;
  } else // handle the other case
  {
    discard;
  }
}
`);
    ...
    it("preserves multi-line block comment", async () => {
      const input = `/*
...
`;
    ...
    it("preserves inline trailing comment on statement", async () => {
      const input = `fn f() {
...
`;
    ...
    it("preserves comment between top-level declarations", async () => {
      const input = `const A = 1;
...
`;
    ...
    it("is idempotent for comments in various positions", async () => {
      await assertIdempotentAtAllWidths(`// this is the main function
@vertex
fn main() -> @builtin(position) vec4f {
  return vec4f(0.0);
}
`);
      await assertIdempotentAtAllWidths(`struct S {
  x: f32, // x coord
  y: f32, // y coord
}
`);
  ...
  describe("idempotency", () => {
    it("is idempotent for simple function", async () => {
      await assertIdempotentAtAllWidths("@vertex fn main() -> @builtin(position) vec4f { return vec4f(0.0); }");
    });

    it("is idempotent for struct", async () => {
      await assertIdempotentAtAllWidths("struct V { @location(0) pos: vec3f, @location(1) col: vec4f, }");
    });

    it("is idempotent for directives", async () => {
      await assertIdempotentAtAllWidths("enable f16;");
    });
